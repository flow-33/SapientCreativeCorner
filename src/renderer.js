(() => {
  // Config is exposed on window by config.js module
  async function loadConfig() { return window.__VO_CFG__; }
  const launcher = document.getElementById('launcher');
  const viewport = document.getElementById('app-viewport');
  // No separate header/home button; this screen is home
  ...
  function openLauncher() {
    ...
    if (logo && logo.parentElement !== launcher) {
      // in case DOM moved; ensure it belongs to launcher
  ...
  const defaultProject = {
    title: 'Creative Corner Portfolio',
    url: 'https://example.com',
    details:
      '<p><strong>Role:</strong> Design & Development</p><p><strong>Team:</strong> Alice, Bob, You</p><p><strong>Notes:</strong> Replace this with project-specific commentary.</p>',
  };
  ...
  // Basic keyboard shortcuts for Phase 1
  ...
  async function fetchWithWeeklyCache(name, url, maxAgeMs) {
    // For local JSON inside data/, bypass cache to reflect immediate edits
    const isLocalDataFile = typeof url === 'string' && url.startsWith('data/');
    if (isLocalDataFile) {
  ...
  // Wire launcher buttons (now two: portfolio and experiments)
